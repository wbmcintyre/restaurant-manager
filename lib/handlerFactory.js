...
export async function deleteOne(req, res, collection, filter) {
  try {
    filter = objectIdForFilter(filter);
    const existingItem = await getOneInCollection(collection, filter);
    if (existingItem) {
      //if existingItem has an image, delete that too
      if (existingItem.image) {
        const filepath = path.resolve(
          ".",
          "img",
          collection,
          path.basename(existingItem.image)
        );
...
export function filterBody(body, filterFor) {
  ...
  for (const [key, value] of Object.entries(body)) {
    if (filterFor.includes(key) && body[key]) {
