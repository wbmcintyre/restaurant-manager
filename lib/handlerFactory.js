...
export async function getAll(req, res, collection) {
  try {
    const allItems = await getAllInCollection(collection);
    res.status(200).json({
      allItems,
    });
  } catch (err) {
...
export async function createOne(
  ...
  if (filteredBody) {
    try {
      if (existFilterFor) {
        existingInCollection(collection, {
      ...
      insertOneInCollection(collection, filteredBody);
